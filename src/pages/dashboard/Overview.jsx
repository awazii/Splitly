...
export const Overview = () => {
  ...
  initial="hidden"
  animate="visible"
  className="overview-cards flex justify-between gap-2 h-full"
>
  {data.map((item, index) => (
    <div
      key={index}
      className="overview-card p-4 bg-white shadow-md rounded-lg flex-1 flex flex-col justify-center items-center"
    >
      <motion.div
      ...
      >
        <div
          className="icon mb-2 size-18 center-flex rounded-full"
        ...
        {item.detail !== "No groups available" && (
          <button
            className="text-primary px-2 py-1 rounded-md text-sm cursor-pointer font-semibold"
