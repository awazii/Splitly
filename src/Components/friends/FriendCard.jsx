...
const Card = ({ friend }) => {
  ...
  const [show, setshow] = useState({ back: false })
  const [flip, setflip] = useState(false)
  const [pin, setpin] = useState(false)
  const actions=[{ svg: TbListDetails, bg: "bg-green-600", label: "Details" }, { svg:!pin ? TbPinnedOff : TbPinnedFilled, bg: "bg-primary", label: pin ? "Unpin" : "Pin" },]
  return (
    <StyledWrapper>
      <div className="card ">
              ...
              <div className="h-42 center-flex w-full  flex-col pt-5">
                <div className="flex items-center py-3 w-full">  
                  <div className='about-f center-flex w-80 gap-3 px-5 '>
                      ...
                      <img className='Img-c' src={friend.profilePic} alt="" />
                    </div> 
                    <div className="info h-10 flex items-start justify-center flex-col flex-1">
                      <h3 className="name text-text-primary font-bold ">{friend.name}</h3>
 ...
 {actions.map((action, index) => (
                      <button key={index} className={`group flex items-center  h-8 min-w-8 px-1.5 rounded-lg cursor-pointer ${action.bg}`} onClick={(()=>{
                        index===1 ? setpin(!pin) : Navigation("/Friends/"+friend.id)
                      })}>
            ...
            <div className="back-content flex flex-col items-center">
              <div className='grid  h-50 w-full p-2 grid-rows-3 grid-cols-2 gap-1'>
                ...
                <div className=' col-span-2 p-2 border-t-1 border-b-light'>
                  ...
                  <h5 className='text-text-secondary text-sm'>{friend.name} Paid 300pkr for Food</h5>
